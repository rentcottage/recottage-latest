...
const DEFAULT_TEXTS: Required<BookingTexts> = {
  ...
  modifyTimeBtn: 'Modify time',
  submitBtn: 'Confirm Booking',
  submittingBtn: 'Booking...',
  successMessage: "Booking confirmed! We'll contact you shortly.",
...
export function BookingCalendar({
  ...
  return (
    <div style={containerStyle} className="bg-white rounded-2xl border border-gray-100 p-6 md:p-8 max-w-xl mx-auto">
      ...
      {step === 0 && (
        <div>
          <h3 className="text-lg font-semibold text-gray-900 mb-1">{t.stepDateTitle}</h3>
          <p className="text-sm text-gray-400 mb-6">{t.selectDatePrompt}</p>
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            <span className="font-semibold text-gray-900 text-sm">
              {calYear > 0 && calMonth > 0 ? `${t.monthNames[calMonth - 1]} ${calYear}` : ''}
            </span>
            <button
              onClick={nextMonth}
              className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors cursor-pointer"
      ...
      {step === 1 && selectedDate && (
        <div>
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900">{t.stepTimeTitle}</h3>
          ...
          {slotsForDate.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 text-sm">{t.noSlotsAvailable}</p>
            </div>
          ) : (
              ...
              {slotsForDate.map((slot, idx) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 mt-2">
                    ...
                    <span className="text-xs text-green-600">{t.availableLegend}</span>
                  </div>
                </button>
              ))}
            </div>
      ...
      {step === 2 && selectedSlot && (
        <div>
          <h3 className="text-lg font-semibold text-gray-900 mb-6">{t.stepFormTitle}</h3>

          {/* Read-only review */}
          <div className="bg-gray-50 rounded-xl p-4 mb-6 border border-gray-100">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-xs text-gray-400 mb-1">{t.selectedDateLabel}</p>
                <p className="text-sm font-semibold text-gray-900">{selectedDate}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-1">{t.selectedTimeLabel}</p>
