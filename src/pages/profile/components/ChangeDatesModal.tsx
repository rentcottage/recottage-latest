...
export default function ChangeDatesModal({ booking, onClose, onSuccess }: ChangeDatesModalProps) {
  ...
  const handleSave = async () => {
    setError('');
    if (!checkIn || !checkOut) { setError(t('account.changeDatesModal.selectBothDatesError')); return; }
    if (checkOut <= checkIn) { setError(t('account.changeDatesModal.checkoutAfterCheckinError')); return; }
    if (checkIn < today) { setError(t('account.changeDatesModal.checkinPastError')); return; }
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error ?? t('account.changeDatesModal.submitFailedError'));
        return;
      }
      ...
      setError(t('account.changeDatesModal.networkError'));
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl w-full max-w-md">
          <div className="p-8 text-center">
            ...
            <h3 className="font-semibold text-gray-900 text-base mb-2">{t('account.changeDatesModal.requestSubmittedTitle')}</h3>
            <p className="text-sm text-gray-500 leading-relaxed mb-1">
              {t('account.changeDatesModal.requestPendingPre')} <strong className="text-gray-700 notranslate" translate="no">{booking.property_title}</strong> {t('account.changeDatesModal.requestPendingPost')}
            </p>
            <p className="text-xs text-gray-400 leading-relaxed mb-6">
              {t('account.changeDatesModal.confirmationEmailNote')}
            </p>
            <div className="bg-gray-50 rounded-lg p-3 text-xs text-gray-500 mb-6 text-left space-y-1">
              <div className="flex justify-between">
                <span className="text-gray-400">{t('account.changeDatesModal.requestedCheckIn')}</span>
            ...
            >
              {t('account.changeDatesModal.close')}
            </button>
          </div>
        </div>
      </div>
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl w-full max-w-md">
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <h3 className="font-semibold text-gray-900 text-base">{t('account.changeDatesModal.requestDateChange')}</h3>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1.5">{t('account.changeDatesModal.newCheckIn')}</label>
              <input
                type="date"
                value={checkIn}
                min={today}
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1.5">{t('account.changeDatesModal.newCheckOut')}</label>
              <input
                type="date"
                value={checkOut}
                min={checkIn || today}
          ...
          {nights > 0 && (
            <div className="flex items-center justify-between text-sm bg-red-50 rounded-lg px-4 py-3">
              <span className="text-gray-600">{plural('account.changeDatesModal.nightsCount', nights)}</span>
              {newTotal !== null ? (
                <span className="font-semibold text-gray-900">{t('account.changeDatesModal.newTotalLine', { price: newTotal })}</span>
              ) : (
                booking.total_price && (
                  <span className="text-gray-500">{t('account.changeDatesModal.originalTotalLine', { price: booking.total_price })}</span>
                )
              )}
            </div>
          )}
          ...
          <div className="flex items-start gap-1.5 text-xs text-amber-600 bg-amber-50 rounded-lg px-3 py-2.5">
            ...
            <span>{t('account.changeDatesModal.requestNote')}</span>
          </div>
        ...
        <div className="flex gap-3 p-5 border-t border-gray-100">
          ...
          >
            {t('account.changeDatesModal.cancel')}
          </button>
          <button
            onClick={handleSave}
          ...
          >
            {saving ? (
              <>
                ...
                {t('account.changeDatesModal.submittingEllipsis')}
              </>
            ) : (
              <>
                ...
                {t('account.changeDatesModal.requestDateChangeBtn')}
              </>
            )}
          </button>
        </div>
