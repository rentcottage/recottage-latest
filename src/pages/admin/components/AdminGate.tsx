    ...
    try {
      ...
      if (res.status === 401) {
        ...
        if (next >= MAX_ATTEMPTS) {
          ...
          setError(t('admin.gate.lockedMessage'));
        } else {
          setError(t('admin.gate.incorrectPassword', { count: MAX_ATTEMPTS - next }));
        }
      } else {
        setError(t('admin.gate.verifyFailed'));
      }
    } catch {
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4" style={{ fontFamily: "'Inter', sans-serif" }}>
      <div className="w-full max-w-sm">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-xl font-bold text-gray-900 tracking-tight">{t('admin.gate.title')}</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder={t('admin.gate.passwordPlaceholder')}
            ...
            {(error || lockedUntil) && (
              <div className={`flex items-start gap-2.5 px-4 py-3 rounded-xl text-xs ${
                ...
                <span>
                  {lockedUntil
                    ? t('admin.gate.lockedCountdown', { seconds: remaining })
                    : error}
                </span>
              </div>
            )}
            ...
            >
              ...
              {verifying ? t('admin.gate.verifying') : t('admin.gate.unlock')}
            </button>
          </form>
        </div>

        <p className="text-center text-xs text-gray-400 mt-5">
          {t('admin.gate.sessionNote')}
        </p>
      </div>
    </div>
  );
