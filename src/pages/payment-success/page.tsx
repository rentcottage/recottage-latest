  ...
  if (!bookingId) {
    return (
      <div className="min-h-screen bg-white flex flex-col">
        <SEO
          title={t('payment.success.seo.title')}
          description={t('payment.success.seo.description')}
          noIndex={true}
        ...
        <div className="flex-1 flex items-center justify-center px-4">
          <div className="text-center">
            <p className="text-gray-500 text-sm">{t('payment.success.noBookingInfo')}</p>
            <button
              onClick={() => navigate('/')}
        ...
        <div className="w-full max-w-md">
          ...
          {(verifyStatus === 'loading' || (verifyStatus === 'pending' && pollCount < 10)) && (
            <div className="text-center">
              ...
              <h1 className="text-2xl font-bold text-gray-900 mb-3">
                {verifyStatus === 'loading' ? t('payment.success.verifying') : t('payment.success.processing')}
              </h1>
              <p className="text-gray-500 text-sm leading-relaxed">
                {t('payment.success.confirmingWithBank')}
              </p>
          ...
          {verifyStatus === 'pending' && pollCount >= 10 && (
            <div className="text-center">
              ...
              <h1 className="text-2xl font-bold text-gray-900 mb-3">{t('payment.success.pendingTitle')}</h1>
              <p className="text-gray-500 text-sm leading-relaxed mb-6">
                {t('payment.success.pendingDesc')}
              </p>
              {booking && (
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-left mb-6">
                  <p className="text-xs text-amber-700 font-medium mb-1">{t('payment.success.bookingReference')}</p>
          ...
          {verifyStatus === 'confirmed' && (
            <div>
              <div className="text-center mb-8">
                ...
                <h1 className="text-2xl font-bold text-gray-900 mb-2">{t('payment.success.title')}</h1>
                <p className="text-gray-500 text-sm">
                  {t('payment.success.confirmedDesc')}
                </p>
              </div>
              ...
              {booking && (
                <div className="bg-gray-50 rounded-xl border border-gray-200 overflow-hidden mb-6">
                  <div className="bg-green-500 px-5 py-3">
                    <p className="text-white text-xs font-semibold uppercase tracking-wider">{t('payment.success.bookingConfirmed')}</p>
                  </div>
                  <div className="p-5 space-y-3">
                    <div>
                      <p className="text-xs text-gray-400 mb-0.5">{t('common.cottage')}</p>
                      <p className="text-sm font-bold text-gray-900 notranslate" translate="no">{booking.property_title}</p>
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-2 border-t border-gray-100">
                      <div>
                        <p className="text-xs text-gray-400 mb-0.5">{t('common.checkIn')}</p>
                        <p className="text-sm font-semibold text-gray-900">{formatDate(booking.check_in, lang)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-400 mb-0.5">{t('common.checkOut')}</p>
                        <p className="text-sm font-semibold text-gray-900">{formatDate(booking.check_out, lang)}</p>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3 pt-2 border-t border-gray-100">
                      <div>
                        <p className="text-xs text-gray-400 mb-0.5">{t('payment.success.duration')}</p>
                        <p className="text-sm font-semibold text-gray-900">
                          {t('common.nights', { count: calcNights(booking.check_in, booking.check_out) })}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-400 mb-0.5">{t('common.guestsLabel')}</p>
                        <p className="text-sm font-semibold text-gray-900">{booking.guests}</p>
                      </div>
                    </div>
                    <div className="pt-2 border-t border-gray-100 flex items-center justify-between">
                      <p className="text-xs text-gray-400">{t('payment.success.totalPaid')}</p>
                      <p className="text-lg font-bold text-gray-900">&#x20BE;{booking.total_price}</p>
                    </div>
                    <div className="pt-2 border-t border-gray-100">
                      <p className="text-xs text-gray-400 mb-0.5">{t('payment.success.bookingId')}</p>
              ...
              <div className="flex flex-col gap-3">
                ...
                >
                  {t('payment.success.browseMoreCottages')}
                </button>
              </div>
            </div>
          )}
          ...
          {verifyStatus === 'failed' && (
            <div className="text-center">
              ...
              <h1 className="text-2xl font-bold text-gray-900 mb-3">{t('payment.success.failedTitle')}</h1>
              <p className="text-gray-500 text-sm leading-relaxed mb-6">
                {t('payment.success.failedDesc')}
              </p>
              ...
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className="w-full bg-red-500 text-white py-3 rounded-lg text-sm font-medium cursor-pointer whitespace-nowrap hover:bg-red-600 transition-colors"
                >
                  {t('payment.success.tryBookingAgain')}
                </button>
                <button
                  onClick={() => navigate('/')}
                  className="w-full border border-gray-200 text-gray-600 py-3 rounded-lg text-sm font-medium cursor-pointer whitespace-nowrap hover:bg-gray-50 transition-colors"
