...
export default function CorporateDashboard() {
  ...
  const loadAccount = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error || !data) {
      setGateError('No agency account is linked to this login.');
      setLoading(false);
      return;
    }
    ...
    return (
      <div className="min-h-screen bg-gray-50" style={{ fontFamily: "'Inter', sans-serif" }}>
        ...
        <div className="max-w-xl mx-auto px-6 py-20 text-center">
          ...
          <h1 className="text-xl font-bold text-gray-900 mb-2">Dashboard unavailable</h1>
          <p className="text-sm text-gray-500 mb-6">{gateError ?? 'Your corporate account could not be loaded.'}</p>
          <button
    ...
    <div className="min-h-screen bg-gray-50" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 mb-8">
          <div>
            <div className="text-xs font-semibold text-emerald-700 uppercase tracking-wide mb-1">Agency Dashboard</div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">{account.agency_name}</h1>
            <p className="text-sm text-gray-500 mt-1">
              {account.rep_first_name} {account.rep_last_name} · Tax ID {account.tax_id} · Earning {Number(account.commission_pct).toFixed(0)}% commission
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
          <StatCard label="Bookings made" value={totalBookings.toString()} icon="ri-calendar-check-line" />
          <StatCard label="Confirmed rent revenue" value={`₾${confirmedRevenue.toLocaleString()}`} icon="ri-money-cny-circle-line" />
          <StatCard label={`Your commission (${Number(account.commission_pct).toFixed(0)}%)`} value={`₾${commission.toLocaleString(undefined, { maximumFractionDigits: 2 })}`} icon="ri-percent-line" highlight />
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <span className="text-xs text-gray-400">{totalBookings} total</span>
          </div>
          {bookings.length === 0 ? (
            <div className="px-6 py-16 text-center">
              ...
              <p className="text-xs text-gray-400">Book a cottage on behalf of a client to start earning commission.</p>
            </div>
          ) : (
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">
                    <th className="px-5 py-3">Cottage</th>
