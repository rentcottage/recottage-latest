  ...
  return (
    <div>
      <div className="mb-5 md:mb-6">
        ...
        <p className="text-xs md:text-sm text-gray-400 mt-0.5">{t('host.reviews.subtitle')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 mb-5 md:mb-6">
        <div className="bg-white rounded-card border border-line shadow-card p-3 md:p-5">
          ...
          <div className="flex items-center gap-1 flex-wrap">
            ...
            <span className="text-xs text-gray-400">{t('host.reviews.fromReviews', { count: reviews.length })}</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-card border border-line shadow-card p-3 md:p-5">
          ...
          <p className="text-xs text-gray-400">
            {reviews.length > 0
              ? t('host.reviews.percentOfAll', { percent: Math.round((reviews.filter((r) => r.rating === 5).length / reviews.length) * 100) })
              : t('host.reviews.noReviewsYetShort')}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-card border border-line shadow-card overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16 text-gray-400">
            ...
            <p className="text-xs mt-1">{t('host.reviews.noReviewsHint')}</p>
          </div>
        ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex flex-col items-end gap-1 flex-shrink-0">
                        ...
                        <span className="text-xs text-gray-400">{timeAgo(r.created_at, t)}</span>
                      </div>
                    </div>
                    {r.review_text && (
